...
**Domain:** `Cybersecurity / Email Safety` · **Suite:** `Security Suite` · **Accent:** `#f87171`
...
**Phishing Detector Api** is a standalone, browser-based AI workflow app for **Cybersecurity / Email Safety**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `email`<br>Email | text | Yes | — | Affects input: Email. |
...
| `target_brand`<br>target brand | text | No | — | Affects customization: target brand. |
| `allow_block_lists`<br>allow/block lists | text | No | — | Affects customization: allow/block lists. |
| `siem_webhook`<br>SIEM webhook | text | No | — | Affects customization: SIEM webhook. |
| `safe_browsing_mode`<br>safe browsing mode | select | No | — | Affects customization: safe browsing mode. |
| `evidence_depth`<br>evidence depth | text | No | — | Affects customization: evidence depth. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9145
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9145`. |
...
```bash
curl http://127.0.0.1:9145/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9145/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9145 uvicorn app.main:app --host 0.0.0.0 --port 9145
```
...
- `plugins/phishing-detector-api.json` for plugin metadata.
